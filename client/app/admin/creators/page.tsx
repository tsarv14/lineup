...
export default function AdminCreatorsPage() {
  ...
  const fetchCreators = async () => {
    setLoading(true)
    try {
      // Fetch creators (now returns all storefronts directly)
                ...
                {creator.storefront?.handle && (
                  <div className="flex gap-2">
                    ...
                    <Link
                      ...
                      title="Edit creator's site"
                      onClick={(e) => {
                        // Store the creator's user ID in sessionStorage so we can edit as them
                        sessionStorage.setItem('editingCreatorId', creator._id)
                      }}
                    >
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
